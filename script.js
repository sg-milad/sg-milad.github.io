document.addEventListener('DOMContentLoaded', () => {
  ...
  if (connectButton) {
    connectButton.addEventListener('click', () => {
      ...
      const isConnected = connectDot.style.backgroundColor === 'var(--success-color)' || 
                          getComputedStyle(connectDot).backgroundColor === 'rgb(30, 185, 128)';
      
      if (isConnected) {
        connectDot.style.backgroundColor = 'var(--error-color)';
        connectButton.innerHTML = '<span class="connect-dot"></span> Disconnected';
        setTimeout(() => {
  ...
  const animateOnScroll = () => {
    const elements = document.querySelectorAll('.swap-panel, .project-card, .token-badge');
